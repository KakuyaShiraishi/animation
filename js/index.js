...
(function() {
    ...
    var TRANSFORM_ELEM_EXPR = 'h1, p, li, dt, dd'; // 変形させるコンテンツ以下の対象要素
    
    // ベンダープレフィックスとプロパティ名
    var prefix = Akm2.Vendor.prefix;
    var prefixHyphen = prefix ? '-' + prefix.toLowerCase() + '-' : '';
    var transformKey = getVendorProperty('transform');
    var animationNameKey = getVendorProperty('animationName');
    // コンテンツの共通スタイルオブジェクト
    var contentStyles = {
        active:   { opacity: 1, zIndex: 99 },
        inactive: { opacity: 0.75, zIndex: 0 }
    };
    contentStyles.active[transformKey]   = 'rotateY(0) translateZ(500px)';
    contentStyles.inactive[transformKey] = 'rotateY(0) translateZ(0)';
    ...
    function initialize() {
        ...
        $contents.each(function(i) {
            ...
            // ハッシュの変更でジャンプしてしまわないよう id を変更
            if (id) $content.attr('id', id + '-content');
        ...
        $('a').each(function(i) {
            ...
            if (href && href.search(/^#/) > -1) {
                ...
                $link.on('click', function(e) {
                    ...
                    // 一番上までスクロール
                    $html_body.animate({ scrollTop: 0 }, 200);
        ...
        naviVisible(contentsIdList[currentIndex] !== TOP_ID);
        
        // ナビゲーションの CSS アニメーションを有効にする
        $nav.addClass('transition-enable');

        // コンテンツの状態を設定
        $contents.each(function(i) {
            var css = {};
            var range = Math.min(width, CONTENT_MAX_WIDTH) * 0.5;
            var randInt = Akm2.randInt;
            
            $(this)
                .addClass('inactive')
                .css(contentStyles.inactive)
                // 要素をランダム配置
                .find(TRANSFORM_ELEM_EXPR).each(function() {
                    var $elem = $(this);
                    css[transformKey] =
                        'translateX(' + randInt(range, -range) + 'px) ' +
                        'translateY(' + randInt(range * 0.5, -range) + 'px) ' + // 少し上に行くよう調整
    ...
    function onWindowResize(e) {
        ...
        adjustContainerHeight($container.height());
    }
    ...
    function onContentTransitionEnd(e) {
        ...
        if ($content.hasClass('inactive')) {
            // 現在のコンテンツでなければアニメーションを開始
            $content[0].style[animationNameKey] = 'rot';
        } else {
            // 現在のコンテンツならコンテンツコンテナの高さを調整
            adjustContainerHeight($content.height() + 135);
        }
    }
    ...
    function contentsTransition(targetIndex) {
        ...
        $contents.each(function(i) {
            var $content = $(this),
                content = $content[0]; // jQuery で css の反映が遅れている気がするので一応要素そのものも取得
            
            content.addEventListener(Akm2.Vendor.transitionend, onContentTransitionEnd, false);
            
            if (i === targetIndex) {
                // transition-enable クラスがあるなら削除
                if ($content.hasClass('transition-enable')) {
                    $content.removeClass('transition-enable');
                }
                
                // inactive なら現在の変形の値を取得してアニメーションを停止し値を反映させる
                if ($content.hasClass('inactive')) {
                    var transformValue = getComputedStyle($content[0], '')[transformKey];
                    content.style[animationNameKey] = 'none';
                    content.style[transformKey] = transformValue;
                }
                
                // 強制的にリフロー
                // @see http://level0.kayac.com/#!2012/08/detecting_css_transition_end.php
                content.offsetTop;
                ...
                $content.find(TRANSFORM_ELEM_EXPR).each(function() {
                    $(this).css(css);
                });
        ...
        // ナビゲーションの状態を設定
        naviVisible(contentsIdList[targetIndex] !== TOP_ID);
        
        currentIndex = targetIndex;
    }
    
    function adjustContainerHeight(containerHeight) {
        containerHeight = Math.max(height, containerHeight);
        $container.css('height', containerHeight + 'px');
    }
    
    function naviVisible(show) {
        $nav[show ? 'removeClass' : 'addClass']('hide');
    }
